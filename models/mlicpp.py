...
class MLICPlusPlus(CompressionModel):
    def __init__(self, config, channel=3, **kwargs):
        ...
        self.entropy_parameters_anchor = nn.ModuleList(
            EntropyParametersMLIC(in_dim=M * 2 + slice_ch * 6, out_dim=slice_ch * 2)
            if i
            else EntropyParametersMLIC(in_dim=M * 2, out_dim=slice_ch * 2)
            for i in range(slice_num)
        )
        self.entropy_parameters_nonanchor = nn.ModuleList(
            EntropyParametersMLIC(in_dim=M * 2 + slice_ch * 10, out_dim=slice_ch * 2)
            if i
            else EntropyParametersMLIC(in_dim=M * 2 + slice_ch * 2, out_dim=slice_ch * 2)
            for i in range(slice_num)
        )
