...
from modules.layers import MLP, build_position_index, conv, deconv
...
class LocalContext(nn.Module):
    def __init__(self, dim=32, window_size=5, mlp_ratio=2.0, num_heads=2, qkv_bias=True, qk_scale=None) -> None:
        super().__init__()
        self.H = -1
        self.W = -1
        self.dim = dim
    ...
    def update_resolution(self, H, W, device, mask=None):
        ...
        if self.H != H or self.W != W:
            ...
            attn_mask = attn_mask.masked_fill(attn_mask == 0.0, float(-100.0)).masked_fill(attn_mask == 1, float(0.0))
    ...
    def forward(self, x):
        ...
        q = q_windows.reshape(B, L, self.head_dim, self.num_heads, self.window_size * self.window_size).permute(
            0, 1, 3, 4, 2
        )
        k = k_windows.reshape(B, L, self.head_dim, self.num_heads, self.window_size * self.window_size).permute(
            0, 1, 3, 4, 2
        )
        v = v_windows.reshape(B, L, self.head_dim, self.num_heads, self.window_size * self.window_size).permute(
            0, 1, 3, 4, 2
        )
        ...
        x = (
            (attn @ v)
            .reshape(B, L, self.num_heads, self.window_size, self.window_size, self.head_dim)
            .permute(0, 1, 3, 4, 2, 5)
        )
