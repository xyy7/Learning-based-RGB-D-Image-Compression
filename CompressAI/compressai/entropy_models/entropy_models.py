...
class _EntropyCoder:
    ...
    def __init__(self, method):
        ...
        if method not in available_entropy_coders():
            ...
            raise ValueError(f'Unknown entropy coder "{method}"' f" (available: {methods})")
...
class EntropyModel(nn.Module):
    ...
    def __init__(self, likelihood_bound=1e-9, entropy_coder=None, entropy_coder_precision=16):
        super().__init__()
    ...
    def quantize(self, inputs: Tensor, mode: str, means: Optional[Tensor] = None) -> Tensor:
        ...
        # 如果有广播机制，那是可以进行相减的；训练的时候可能没有使用到pad，但是在测试的时候，如果尺寸不是某种倍数关系，那么就需要使用pad，或者使用center-crop
        # print(outputs.shape, means.shape)
        if means is not None:
            outputs -= means

        outputs = torch.round(outputs)  # train直接加了noise就return了，但是eval是使用round
    ...
    def decompress(self, strings, indexes, means=None):
        ...
        if means is not None:
            ...
            if means.size() != indexes.size() and (means.size(2) != 1 or means.size(3) != 1):
        ...
        for i, s in enumerate(strings):
            values = self.entropy_coder.decode_with_indexes(
                s, indexes[i].reshape(-1).int().tolist(), cdf.tolist(), self._cdf_length.reshape(-1).int().tolist(), self._offset.reshape(-1).int().tolist()
            )
...
class EntropyBottleneck(EntropyModel):
    ...
    *tensorflow/compression*. See the original paper and the `tensorflow
    ...
    def __init__(self, channels, *args, tail_mass=1e-9, init_scale=10, filters=(3, 3, 3, 3), **kwargs):
        ...
        self.quantiles = nn.Parameter(torch.Tensor(channels, 1, 3))  # 128,1,3
        init = torch.Tensor([-self.init_scale, 0, self.init_scale])  # -10,0,10
        self.quantiles.data = init.repeat(self.quantiles.size(0), 1, 1)  # [[-10,0,10]...[-10,0,10]] # 量级

        target = np.log(2 / self.tail_mass - 1)
        self.register_buffer("target", torch.Tensor([-target, 0, target]))  # tensor不会变 [-21,0,21] 为什么不是0-1？ sigmoid([-21,0,21])=[0,1]

    def _get_medians(self):
        medians = self.quantiles[:, :, 1:2]  # 128维通道的中位数
        return medians
    ...
    # L1损失，保证了通过组件计算出来的cdf是个符合[-inf,inf]的分布函数；用来搜索quantile的边界【另外一种没有使用aux-loss，则通过缩紧法来判断quantile的边界】
    def loss(self):
    ...
    def forward(self, x):
        ...
        outputs = self.quantize(values, "noise" if self.training else "dequantize", self._get_medians())
...
class GaussianConditional(EntropyModel):
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def _standardized_quantile(quantile):
        return scipy.stats.norm.ppf(quantile)  # 逆累积分布函数（inverse cumulative distribution function，简称逆CDF或PPF）。给定一个概率（quantile），该函数返回相应的分布值，反映了在正态分布下，具有给定概率的观测值的位置。
    ...
    def update(self):
        multiplier = -self._standardized_quantile(self.tail_mass / 2)  # 6.11
        pmf_center = torch.ceil(self.scale_table * multiplier).int()  # [64]:[1,1565]
        pmf_length = 2 * pmf_center + 1  # [64]:[3,3131]
        max_length = torch.max(pmf_length).item()  # 3131

        device = pmf_center.device
        samples = torch.abs(torch.arange(max_length, device=device).int() - pmf_center[:, None])  # [64,3131] # 生成一个包含从0到 max_length - 1 的整数序列的张量（tensor）
        samples_scale = self.scale_table.unsqueeze(1)
        samples = samples.float()
        samples_scale = samples_scale.float()
        upper = self._standardized_cumulative((0.5 - samples) / samples_scale)  # 重新返回加噪标准正态分布的x范围 Q(y_hat-0.5)
        lower = self._standardized_cumulative((-0.5 - samples) / samples_scale)  # Q(y_hat+0.5)
        pmf = upper - lower  # p(y_hat)=c(y_hat+0.5)-c(y_hat-0.5)=1-Q(y_hat+0.5)-(1-Q(y_hat-0.5))=Q(y_hat-0.5)-Q(y_hat+0.5)

        tail_mass = 2 * lower[:, :1]

        quantized_cdf = torch.Tensor(len(pmf_length), max_length + 2)  # 随机初始化
        quantized_cdf = self._pmf_to_cdf(pmf, tail_mass, pmf_length, max_length)
        self._quantized_cdf = quantized_cdf
    ...
    def forward(self, inputs, scales, means=None):
        ...
        outputs = self.quantize(inputs, "noise" if self.training else "dequantize", means)
        likelihood = self._likelihood(outputs, scales, means)
        if self.use_likelihood_bound:
            likelihood = self.likelihood_lower_bound(likelihood)
        return outputs, likelihood

    # 将超先验预测出来的scale转化成相对应的index
    def build_indexes(self, scales):
        scales = self.lower_bound_scale(scales)
        indexes = scales.new_full(scales.size(), len(self.scale_table) - 1).int()
        for s in self.scale_table[:-1]:
            less = scales <= s
            indexes -= less.int()  # 可能减少1次，也可能减少多次
        # print(indexes.float().mean())
        return indexes
