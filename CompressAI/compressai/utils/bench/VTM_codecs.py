...
class BinaryCodec(Codec):
    ...
    def _run(self, img, quality, return_rec=False, return_metrics=True):
        ...
        img_path = img
        img_par = os.path.dirname(os.path.dirname(img_path))
        os.makedirs(os.path.join(img_par, "bpg_" + str(quality)), exist_ok=True)
        # rec_dir = "depth_rec" if img.find("rgb") == -1 and img.find("color") == -1 else "rgb_rec"
        rec_dir = "rgb_rec" if self.rgb else "depth_rec"
        os.makedirs(os.path.join(img_par, "bpg_" + str(quality), rec_dir), exist_ok=True)
        png_filepath = os.path.join(img_par, "bpg_" + str(quality), rec_dir, str(os.path.basename(img))[:-4] + ".png")
        rec_dir = "rgb_bin" if self.rgb else "depth_bin"
        os.makedirs(os.path.join(img_par, "bpg_" + str(quality), rec_dir), exist_ok=True)
        out_filepath = os.path.join(img_par, "bpg_" + str(quality), rec_dir, str(os.path.basename(img))[:-4] + ".bin")
        ...
        out = {"quality": quality, "img": os.path.basename(img_path), "bpp": bpp_val, "encoding_time": enc_time, "decoding_time": dec_time}
        ...
        print(out)
        modal = "RGB" if img_path.find("depth") == -1 and img_path.find("depth_8bit") == -1 else "Depth"
        with open(os.path.join(img_par, "bpg_" + str(quality), f"{modal}_per_img.json"), "a") as f:
...
class JPEG2000(BinaryCodec):
    ...
    def _get_encode_cmd(self, img, quality, out_filepath):
        cmd = ["ffmpeg", "-loglevel", "panic", "-y", "-i", img, "-vcodec", "jpeg2000", "-pix_fmt", "yuv444p", "-c:v", "libopenjpeg", "-compression_level", quality, out_filepath]
        return cmd
...
class BPG(BinaryCodec):
    ...
    @property
    def name(self):
        return f"BPG {self.bitdepth}b {self.subsampling_mode} {self.encoder} " f"{self.color_mode}"
    ...
    def setup_args(cls, parser):
        ...
        parser.add_argument("-m", choices=["420", "444"], default="444", help="subsampling mode (default: %(default)s)")
        parser.add_argument("-b", choices=["8", "10"], default="8", help="bitdepth (default: %(default)s)")
        parser.add_argument("-c", choices=["rgb", "ycbcr"], default="rgb", help="colorspace  (default: %(default)s)")
        parser.add_argument("-e", choices=["jctvc", "x265"], default="x265", help="HEVC implementation (default: %(default)s)")
    ...
    def _set_args(self, args):
        ...
        self.encoder_path = "/data/chenminghui/libbpg-0.9.8/bpgenc"  # args.encoder_path
        self.decoder_path = "/data/chenminghui/libbpg-0.9.8/bpgdec"
        return args

    def _get_encode_cmd(self, img, quality, out_filepath):
        if not 0 <= quality <= 51:
            raise ValueError(f"Invalid quality value: {quality} (0,51)")
        cmd = [self.encoder_path, "-o", out_filepath, "-q", str(quality), "-f", self.subsampling_mode, "-e", self.encoder, "-c", self.color_mode, "-b", self.bitdepth, img]
        return cmd
...
class VTM(Codec):
    ...
    def setup_args(cls, parser):
        super().setup_args(parser)
        parser.add_argument("-b", "--build-dir", type=str, default="/data/chenminghui/VVCSoftware_VTM-master/bin", help="VTM build dir")
        parser.add_argument("-c", "--config", type=str, default="/data/chenminghui/VVCSoftware_VTM-master/cfg/encoder_intra_vtm.cfg", help="VTM config file")
        parser.add_argument("--rgb", action="store_true", help="Use RGB color space (over YCbCr)")  # 使用rgb色彩格式压缩，还是使用yuv色彩格式压缩
    ...
    def _set_args(self, args):
        ...
        self.rgb = args.rgb
        self.input_chroma_format = "444" if args.modal=="RGB" else "400"
        self.mode = "RGB" if args.modal=="RGB" else "L"

        return args

    def _run(self, img_path, quality, return_rec=False, return_metrics=True):
        if not 0 <= quality <= 63:
        ...
        arr = np.asarray(read_image(img_path, self.mode))
        root = os.path.dirname(os.path.dirname(img_path))
        imgname = os.path.basename(img_path)[:-4]
        
        # VTM_37
        os.makedirs(os.path.join(root, "vtm_" + str(quality)), exist_ok=True) 
        # VTM_37/rgb_rec VTM_37/rgb_bin
        rec_dir = "rgb_rec" if self.mode=="RGB" else "x_rec"
        png_rec_dir = os.path.join(root, "vtm_" + str(quality), rec_dir)
        bin_dir = os.path.join(root, "vtm_" + str(quality), rec_dir.replace("rec","bin"))
        os.makedirs(png_rec_dir, exist_ok=True)
        os.makedirs(bin_dir, exist_ok=True)

        input_path = os.path.join(bin_dir,imgname+".yuv") 
        bin_filepath = os.path.join(bin_dir,imgname+".bin")
        output_path1 = os.path.join(png_rec_dir,imgname+".yuv")
        output_path2 = os.path.join(png_rec_dir,imgname+".png")

        if self.mode == "RGB":
            arr = arr.transpose((2, 0, 1))  # color channel first

        if not self.rgb and self.mode=="RGB":
        ...
        cmd = [
            ...
            "-f",
            "1",
            f"--InputChromaFormat={self.input_chroma_format}",
            "--InputBitDepth=8",
        ]

        print(cmd," #finish#")

        if self.rgb and self.mode=="RGB":
        ...
        # Decode
        cmd = [self.decoder_path, "-b", bin_filepath, "-o", output_path1, "-d", 8]
        if self.rgb and self.mode=="RGB":
        ...
        # Compute PSNR
        rec_arr = np.fromfile(output_path1, dtype=np.uint8)  # 不是保存成图片，而是保存成二进制或者numpy的形式
        ...
        bpp = filesize(bin_filepath) * 8.0 / (height * width)

        # Cleanup
        # os.unlink(yuv_path)
        # os.unlink(out_filepath)

        out = {"quality": quality, "img": os.path.basename(img_path), "bpp": bpp, "encoding_time": enc_time, "decoding_time": dec_time}
        ...
        print(out)
        modal = "RGB" if self.mode=="RGB" else "x"
        with open(os.path.join(root, "vtm_" + str(quality), f"{modal}_per_img.json"), "a") as f:
            json.dump(out, f, indent=2)

        print("rec_arr.shape", rec_arr.shape, output_path2)
        if self.mode=="RGB":
            rec_arr = rec_arr.transpose(1, 2, 0)
        rec = Image.fromarray((rec_arr.clip(0, 1) * 255.0).astype(np.uint8))
        rec.save(output_path2)
...
class HM(Codec):
    ...
    def setup_args(cls, parser):
        super().setup_args(parser)
        parser.add_argument("-b", "--build-dir", type=str, required=True, help="HM build dir")
        parser.add_argument("-c", "--config", type=str, required=True, help="HM config file")
    ...
    def _run(self, img, quality, return_rec=False, return_metrics=True):
        ...
        mode = "RGB" if self.rgb else "L" # 不支持超过8bit
        arr = np.asarray(read_image(img, mode))
...
class AV1(Codec):
    ...
    def setup_args(cls, parser):
        super().setup_args(parser)
        parser.add_argument("-b", "--build-dir", type=str, required=True, help="AOM binaries dir")
