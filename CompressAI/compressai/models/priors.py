...
class CompressionModel(nn.Module):
    ...
    def __init__(self, entropy_bottleneck_channels, init_weights=True, **kwargs):  # 可以不用，但不能没有，否则相当于前面都废掉
        super().__init__()
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if self.entropy_bottleneck is not None:
            update_registered_buffers(
                self.entropy_bottleneck, "entropy_bottleneck", ["_quantized_cdf", "_offset", "_cdf_length"], state_dict
            )
...
class ScaleHyperprior(CompressionModel):
    ...
    def __init__(self, N, M, **kwargs):
        ...
        self.h_a = nn.Sequential(
            conv(M, N, stride=1, kernel_size=3), nn.ReLU(inplace=True), conv(N, N), nn.ReLU(inplace=True), conv(N, N)
        )
...
class JointAutoregressiveHierarchicalPriors(MeanScaleHyperprior):
    ...
    @classmethod
    def from_state_dict(cls, state_dict, ch=3):
        """Return a new model instance from `state_dict`."""
        N = state_dict["g_a.0.weight"].size(0)
        M = state_dict["g_a.6.weight"].size(0)
        try:
            print("from state dict try")
            net = cls(N, M, ch=ch)
        except:
            print("from state dict except")
            net = cls(N, M)
